...
const ComparingButton2 = ({
  ...
  const { data: product2 } = useQuery(
    reCompareSecondOptions(productId2, accessToken),
  );
  ...
  const [firstName, setFirstName] = useState(product1 ? product1.name : '');
  const [secondName, setSecondName] = useState(product2 ? product2.name : '');
...
export default ComparingButton2;
