...
import { ExempleState } from "./storeType";

// 0. storeType.ts에서 타입 선언
// 1. create를 이용해 스토어를 생성
// 2. create<타입>()(...) 타입스크립트에서는 반드시 () 하나 더 넣어야함!
// 3. 필요에 따라 미들웨어를 적용한다.
const useExempleStore = create<ExempleState>()((set) => ({
  exemple: "예시",
  setExemple: (newExemple) => set((state) => ({ exemple: newExemple })),
}));
...
// devtools((set) => ({...}), {name: "ExempleStore"})
...
// persist((set) => ({...}), {name: "exemple", storage: createJSONStorage(() => localStorage/sessionStorage), partialize: (state) => ({ exemple: state.exemple }), })

// 둘다 필요시 devtools를 제일 겉에서 감싸는걸 추천한다고함. ex) devtools(persist(...))

// 어떻게 사용해?
// const exemple = useExempleStore((state) => state.exemple);
// const setExemple = useExempleStore((state) => state.setExemple);
// 위 예시 코드처럼 사용하려는 곳에서 훅 처럼 호출해서 사용하면 된다.
// 주의할점은 구조분해 할당을 이용해 한번에 호출하면 안된다. 이 경우 서로 영향을 줘서 불필요한 부분까지 재렌더링을 유발할 수 있다고한다.
// Bad Code:
// const {exemple, setExemple} = useExempleStore();
